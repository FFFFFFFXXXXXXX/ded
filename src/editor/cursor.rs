...
impl CursorMove {
    pub(crate) fn next_cursor(
        &self,
        CursorPosition { row, col }: CursorPosition,
        lines: &[String],
        viewport: &Viewport,
    ) -> Option<(usize, usize)> {
        use CursorMove::*;
        ...
        match self {
            ...
            WordEnd => {
                ...
                if let Some(col) = word::find_inclusive_end_forward(&lines[row], col + 1) {
                    Some((row, col))
                    ...
                    loop {
                        ...
                        row += 1;
                        if let Some(col) = word::find_inclusive_end_forward(&lines[row], 0) {
                            break Some((row, col));
                        }
                    }
                }
            ...
            WordForward => {
                if let Some(col) = word::find_exclusive_end_forward(&lines[row], col) {
                    Some((row, col))
                } else {
                    Some((row + 1, 0))
                }
            ...
            WordBack => {
                if let Some(col) = word::find_start_backward(&lines[row], col) {
                    Some((row, col))
                } else if row == 0 {
                    Some((row, col))
                } else {
